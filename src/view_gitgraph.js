...
function get_git_log_and_plot() {
  ...
  const command = `git -C ${directory} log --${git_log_order} --all --pretty=format:'¡¨¡%h¡¨¡%p¡¨¡%d¡¨¡%s¡¨¡%ci¡¨¡%an¡¨¡' --abbrev-commit --date=short --decorate=full`
  exec.command(command, function(stdout, stderr) {
    ...
    if (stderr.match(/^(error|fatal): /)) {
      ...
      if (0) {
        // This is useable, and looks better if the src/css/styles.css is avilable
        var fs = require('fs')
        var filename = `temp/file-${new Date().getTime()}.html`
